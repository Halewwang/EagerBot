...
export async function exchangeRefreshTokenOverHttp(input: {
  ...
  if (!response.ok) {
    ...
    throw new TokenRefusedError(
      `供应商不愿续订此访问权限（${response.status}）。${code ? `（${code}）` : ""}`,
      code,
    );
  }
  ...
  if (!body) {
    throw new McpServerError("供应商续订时返回的内容不是令牌。");
  }
...
export function createPluginStore(options: PluginStoreOptions) {
  ...
  async function connectionTokenFor(
    ...
    if (!actorId) {
      throw new PluginRefusedError(
        `${row.id} 会以提问者身份响应，但此次运行没有归属到任何人。`,
        null,
      );
    }
    ...
    const noClient = registrationUrl
      ? `${title} 尚未为此部署注册 OAuth 客户端，因此无法调用。请在设置中重新连接 ${title}，部署会在连接时自动向服务提供商注册。`
      : `${title} 尚未为此部署注册 OAuth 客户端，因此无法调用。管理员需要添加一个。`;
    const unusableClient = registrationUrl
      ? `${title} 没有可供此部署使用的 OAuth 客户端。请在设置中重新连接 ${title}，部署会在下次连接时自动向服务提供商注册。`
      : `${title} 没有可供此部署使用的 OAuth 客户端。管理员需要重新添加一个。`;
    /**
     ...
     */
    const clientReplaced = `${title} 不再识别此部署的 OAuth 客户端，因此无法调用。部署已重新注册，请在设置中重新连接 ${title}。`;
  ...
  async function requireCredentialOfKind(
     ...
     */
    if (!kind) {
      throw new CustomServerRefusedError(`${serverTitle} 添加时不需要凭据。`);
    }
  ...
  return {
    ...
    async addCustomServer(input: {
      ...
      if (
        ...
        throw new CustomServerRefusedError(
          `${input.id} 已存在于其他地址且持有凭据。请先移除它，再使用新地址应使用的令牌重新添加。`,
        );
      }
        ...
        if (stranded.length > 0) {
          await recordAuditEvent(auditStore, {
            ...
            payload: {
              ...
              note: "此智能体持有该工具，但服务器已不再提供，因此不会向任何模型提供。服务器恢复提供后会再次显示。",
            },
          });
        }
        ...
        if (unlisted.length > 0) {
          await recordAuditEvent(auditStore, {
            ...
            payload: {
              ...
              server: serverId,
              tools: unlisted,
              note: "此服务器提供了这些工具，但它们未列入已审核的写入列表，因此会作为读取工具提供给模型。此服务提供商没有为该列表提供只读范围；其中任何会写入的工具都应添加到条目中。",
            },
          });
        }
