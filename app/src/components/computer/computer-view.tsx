      ...
      {settled ? (
        <>
          ...
          */}
          {page?.url ? (
            <>
              <span className="font-medium">{page.title || "网页"}</span>
              <span className="break-all">{hostOf(page.url)}</span>
              <span>此轮操作期间已打开。屏幕现已显示后续页面。</span>
            </>
          ) : (
            /*
        ...
        <>
          ...
          <span>{problem}</span>
          <span>智能体可能仍在工作。管理员可以检查它的电脑是否正在运行。</span>
        </>
      ) : blankBrowser ? (
        <span>智能体尚未打开网页。</span>
      ) : (
        <span>正在等待智能体的屏幕…</span>
      )}
    </span>
  );
}
      ...
      <figure className="overflow-hidden rounded-2xl border">
        ...
        {!driving && !settled && control?.requested ? (
          <div className="flex items-start justify-between gap-3 border-t bg-amber-500/10 px-3 py-2 text-sm">
            <span>
              <strong className="font-medium">智能体需要你的帮助。</strong>{" "}
              {control.reason}
            </span>
            <button
              type="button"
        ...
        {control?.secretWanted ? (
          ...
          >
            <label className="block" htmlFor="openbot-secret">
              <span className="font-medium">智能体需要 </span>
